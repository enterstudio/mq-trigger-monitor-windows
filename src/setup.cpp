...
int main(int argc, char *argv[]) {
	...
	// check that we are running under NT
	// Wow, this is old.  Commenting out, because, wow this is old.

//	version = GetVersion();
//	if ((version & 0x80000000) != 0) {
//		ErrorHandler("Stopping, this must be installed on Windows NT", 1);
//	}
	...
	ret=RegCreateKeyEx(HKEY_LOCAL_MACHINE, strBuf, 0, NULL,
			REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &keyHandle,
			&disposition);
	...
	ret=RegSetValueEx(keyHandle, "TypesSupported", 0, REG_DWORD, (LPBYTE)
			&eventTypes, sizeof(DWORD));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	if (categoryCount) {
		...
		ret=RegSetValueEx(keyHandle, "CategoryCount", 0, REG_DWORD, (LPBYTE)
				&categoryCount, sizeof(DWORD));
		if (ret != ERROR_SUCCESS)
			ErrorHandler("Unable to add value to key", GetLastError());
	...
	if (strcmp(serviceUserid, DEFAULTID)) {
		...
		newService = CreateService(scm, serviceName, serviceName,
				...
				PathProgram, // eg "c:\winnt\xxx.exe"
				0, // load order group
				0, // ID
				0, // dependencies
		...
		newService = CreateService(scm, serviceName, serviceName,
				SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS
						| SERVICE_INTERACTIVE_PROCESS, SERVICE_AUTO_START, // when to start
				SERVICE_ERROR_NORMAL, // severity if doesnt start
				PathProgram, // eg "c:\winnt\xxx.exe"
				0, // load order group
				0, // ID
	...
	ret=RegSetValueEx(keyHandle, TRIGGERQUEUENAME, 0, REG_MULTI_SZ, (LPBYTE)
			&QueueName, qindx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	ret=RegSetValueEx(keyHandle, SERVICENAME, 0, REG_MULTI_SZ, (LPBYTE)
			&ServiceName, sindx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the TriggerQueueQmrName value to key
	ret=RegSetValueEx(keyHandle, TRIGGERQUEUEMGRNAME, 0, REG_MULTI_SZ, (LPBYTE)
			&QueueMgrName, qmgrindx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the NotesIni value to key
	ret=RegSetValueEx(keyHandle, NOTESINI, 0, REG_MULTI_SZ, (LPBYTE) &NotesIni,
			niindx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	ret=RegSetValueEx(keyHandle, LOCLADDR, 0, REG_MULTI_SZ, (LPBYTE) &Locladdr,
			locl_indx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	ret=RegSetValueEx(keyHandle, SENDDATA, 0, REG_MULTI_SZ, (LPBYTE) &SendData,
			sndd_indx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	ret=RegSetValueEx(keyHandle, SENDEXIT, 0, REG_MULTI_SZ, (LPBYTE) &SendExit,
			snde_indx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	ret=RegSetValueEx(keyHandle, CHANNELUID, 0, REG_MULTI_SZ, (LPBYTE) &ChannelUserId,
			chlui_indx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	ret=RegSetValueEx(keyHandle, CHANNELPW, 0, REG_MULTI_SZ, (LPBYTE) &ChannelPassword,
			chlpw_indx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	ret=RegSetValueEx(keyHandle, MA7K_MQCD_VERSION, 0, REG_MULTI_SZ, (LPBYTE)
			&mqcdVersion, mqcd_indx);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the WaitInterval value to key
	ret=RegSetValueEx(keyHandle, WAITINTERVAL, 0, REG_DWORD, (LPBYTE)
			&WaitIntervalDefault, sizeof(WaitIntervalDefault));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the LongRty value to key
	ret=RegSetValueEx(keyHandle, LONGRTY, 0, REG_DWORD, (LPBYTE)
			&LongRtyDefault, sizeof(LongRtyDefault));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the LongTmr value to key
	ret=RegSetValueEx(keyHandle, LONGTMR, 0, REG_DWORD, (LPBYTE)
			&LongTmrDefault, sizeof(LongTmrDefault));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the ShortRty value to key
	ret=RegSetValueEx(keyHandle, SHORTRTY, 0, REG_DWORD, (LPBYTE)
			&ShortRtyDefault, sizeof(ShortRtyDefault));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the ShortTmr value to key
	ret=RegSetValueEx(keyHandle, SHORTTMR, 0, REG_DWORD, (LPBYTE)
			&ShortTmrDefault, sizeof(ShortTmrDefault));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());

	// add the EventLevel value to key
	ret=RegSetValueEx(keyHandle, EVENTLEVEL, 0, REG_DWORD, (LPBYTE)
			&EventLevelDefault, sizeof(EventLevelDefault));
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
	...
	ret=RegSetValueEx(keyHandle, AGENTREDIR, 0, REG_SZ, (LPBYTE)
			&AgentRedirDefault, strlen(AgentRedirDefault)+1);
	if (ret != ERROR_SUCCESS)
		ErrorHandler("Unable to add value to key", GetLastError());
...
int processini(char * configFile) {
	...
	tempPw = (char*) malloc(MQ_CSP_PASSWORD_LENGTH+MQ_CSP_PASSWORD_LENGTH+1);
...
//		printf ("Buff is currently %c and rlen is %d\n",buf[0],rlen);
		...
		if (*buf!='*'&& i < rlen-1) {
			...
			pStrBuff = buf;
			pEndBuff = buf + rlen -1;
			while ( (*pStrBuff == ' ' || *pStrBuff == '\t') && pStrBuff
					< pEndBuff)
				pStrBuff ++; // scan to first non-blank
			while ( (*pEndBuff == ' ' || *pEndBuff == '\t') && pStrBuff
					< pEndBuff)
			...
			if (!memcmp(buf, "Global:", 7)) {
				...
				if (recordsread++ == MAXTHREADS) {
					printf(
							"Too many stanzas encountered, maximum is %d\n",
							MAXTHREADS);
					rc = 4;
					break;
				}
			...
			if (inThread) {
				...
				if (!memcmp(buf, TRIGGERQUEUENAME, strlen(TRIGGERQUEUENAME))
					...
					strncpy(t[recordsread-1].q, buf +(strlen(TRIGGERQUEUENAME)
							+1), rlen - (strlen(TRIGGERQUEUENAME)+1));
				...
				/**** v140 stuff here .... */
				else if (!memcmp(buf, CONNAME, strlen(CONNAME)) && *(buf
						+strlen(CONNAME))== '=') {
					strncpy(t[recordsread-1].conname, buf+(strlen(CONNAME)+1),
							rlen - (strlen(CONNAME)+1));
				} else if (!memcmp(buf, CHANNEL, strlen(CHANNEL)) && *(buf
						+strlen(CHANNEL))== '=') {
					strncpy(t[recordsread-1].channel, buf+(strlen(CHANNEL)+1),
							rlen - (strlen(CHANNEL)+1));
				} else if (!memcmp(buf, LOCLADDR, strlen(LOCLADDR)) && *(buf
						+strlen(LOCLADDR))== '=') {
					strncpy(t[recordsread-1].locladdr,
							buf+(strlen(LOCLADDR)+1), rlen - (strlen(LOCLADDR)
									+1));
				} else if (!memcmp(buf, RCVDATA, strlen(RCVDATA)) && *(buf
						+strlen(RCVDATA))== '=') {
					strncpy(t[recordsread-1].rcvData, buf+(strlen(RCVDATA)+1),
							rlen - (strlen(RCVDATA)+1));
				} else if (!memcmp(buf, SCYDATA, strlen(SCYDATA)) && *(buf
						+strlen(SCYDATA))== '=') {
					strncpy(t[recordsread-1].scyData, buf+(strlen(SCYDATA)+1),
							rlen - (strlen(SCYDATA)+1));
				} else if (!memcmp(buf, SENDDATA, strlen(SENDDATA)) && *(buf
						+strlen(SENDDATA))== '=') {
					strncpy(t[recordsread-1].sendData,
							buf+(strlen(SENDDATA)+1), rlen - (strlen(SENDDATA)
									+1));
				} else if (!memcmp(buf, RCVEXIT, strlen(RCVEXIT)) && *(buf
						+strlen(RCVEXIT))== '=') {
					strncpy(t[recordsread-1].rcvExit, buf+(strlen(RCVEXIT)+1),
							rlen - (strlen(RCVEXIT)+1));
				} else if (!memcmp(buf, SCYEXIT, strlen(SCYEXIT)) && *(buf
						+strlen(SCYEXIT))== '=') {
					strncpy(t[recordsread-1].scyExit, buf+(strlen(SCYEXIT)+1),
							rlen - (strlen(SCYEXIT)+1));
				} else if (!memcmp(buf, SENDEXIT, strlen(SENDEXIT)) && *(buf
						+strlen(SENDEXIT))== '=') {
					strncpy(t[recordsread-1].sendExit,
							buf+(strlen(SENDEXIT)+1), rlen - (strlen(SENDEXIT)
									+1));
				} else if (!memcmp(buf, USERID, strlen(USERID)) && *(buf
						+strlen(USERID))== '=') {
					strncpy(t[recordsread-1].userid, buf+(strlen(USERID)+1),
							rlen - (strlen(USERID)+1));
				} else if (!memcmp(buf, SSLCIPH, strlen(SSLCIPH)) && *(buf
						+strlen(SSLCIPH))== '=') {
					strncpy(t[recordsread-1].sslciph, buf+(strlen(SSLCIPH)+1),
							rlen - (strlen(SSLCIPH)+1));
				} else if (!memcmp(buf, SSLPEER, strlen(SSLPEER)) && *(buf
						+strlen(SSLPEER))== '=') {
					strncpy(t[recordsread-1].sslpeer, buf+(strlen(SSLPEER)+1),
							rlen - (strlen(SSLPEER)+1));
				}

				// else if (!memcmp(buf, TRPTYPE, strlen(TRPTYPE)) && *(buf+strlen(TRPTYPE))== '=' ) {
				//   strncpy(t[recordsread-1].trptype, buf+(strlen(TRPTYPE)+1), rlen - (strlen(TRPTYPE)+1) );
				// }
				else if (!memcmp(buf, HBINT, strlen(HBINT)) && *(buf
						+strlen(HBINT))== '=') {
					strncpy(t[recordsread-1].hbint, buf+(strlen(HBINT)+1), rlen
							- (strlen(HBINT)+1));
				} else if (!memcmp(buf, KAINT, strlen(KAINT)) && *(buf
						+strlen(KAINT))== '=') {
					strncpy(t[recordsread-1].kaint, buf+(strlen(KAINT)+1), rlen
							- (strlen(KAINT)+1));
				}  else if (!memcmp(buf, CHANNELUID, strlen(CHANNELUID)) && *(buf
						+strlen(CHANNELUID))== '=') {
					strncpy(t[recordsread-1].channelUserId, buf+(strlen(CHANNELUID)+1), rlen
							- (strlen(CHANNELUID)+1));
						//	prompt for password!
					memset(tempPw,'\0',sizeof(tempPw));
				    ...
				    strcpy(t[recordsread-1].channelPassword,tempVal) ;
						...
						strlen(MA7K_MQCD_VERSION)) && *(buf
						+strlen(MA7K_MQCD_VERSION))== '=') {
					strncpy(t[recordsread-1].mqcdversion, buf
							+(strlen(MA7K_MQCD_VERSION)+1), rlen
							- (strlen(MA7K_MQCD_VERSION)+1));
				}
			...
			if (inService) {
				if (!memcmp(buf, SERVICENAME, strlen(SERVICENAME)) && *(buf
						+strlen(SERVICENAME))== '=') {

					strncpy(t[recordsread-1].service, buf+(strlen(SERVICENAME)
							+1), rlen - (strlen(SERVICENAME)+1));
					...
					t[recordsread-1].q[0]=PLACEHOLDER;
						...
						strlen(SERVICEQUEUEMGRNAME)) && *(buf
						+strlen(SERVICEQUEUEMGRNAME))== '=') {
					strncpy(t[recordsread-1].qm, buf
							+(strlen(SERVICEQUEUEMGRNAME)+1), rlen
							- (strlen(SERVICEQUEUEMGRNAME)+1));
				} else if (!memcmp(buf, CONNAME, strlen(CONNAME)) && *(buf
						+strlen(CONNAME))== '=') {
					strncpy(t[recordsread-1].conname, buf+(strlen(CONNAME)+1),
							rlen - (strlen(CONNAME)+1));
	...
	if (!recordsread) {
		printf("No Thread stanza's found, there must be at least one Thread stanza\n");
		return (5);
	}
	...
	if (servUseridSpecified && !servPasswordSpecified) {
		printf("Please enter the service password (the password will NOT be displayed as you type!!!)\n");
	    HANDLE hStdin = GetStdHandle(STD_INPUT_HANDLE);
	    DWORD mode = 0;
	    GetConsoleMode(hStdin, &mode);
	    SetConsoleMode(hStdin, mode & (~ENABLE_ECHO_INPUT));
	    ...
	    SetConsoleMode(hStdin, mode & (ENABLE_ECHO_INPUT));
		// change the \n to \0
		printf("Service password ssucessfully enetered!!!)\n");
	...
	for (i=0; i<recordsread; i++) {
		...
		if (!(t[i].q[0])&&!(t[i].q[0]==PLACEHOLDER)) {
			printf("Error, a Thread stanza was found which did not name an initiation queue\n");
			return (2);
		} else {
			printf("\nThread %d, TriggerQueueName=\"%s\"\n", i, t[i].q);
		}
		if (!(t[i].service[0]) && !(t[i].service[0]==PLACEHOLDER)) {
			printf("Error, a Service stanza was found which did not name the service\n");
			return (2);
		} else {
		...
		makeGoodForRegistry(Sslpeer, t[i].sslpeer, sizeof(t[0].sslpeer),
				&sslp_indx);
		// makeGoodForRegistry( Trptype, t[i].trptype, sizeof(t[0].trptype), &trpt_indx ); 
		...
		makeGoodForRegistry(ChannelUserId, t[i].channelUserId, sizeof(t[0].channelUserId), &chlui_indx);
		makeGoodForRegistry(ChannelPassword,
				t[i].channelPassword,
				sizeof(t[0].channelPassword),
				&chlpw_indx);
